    ...
    with get_session() as session:
        ...
        with Progress(
            ...
            if notion or google or apple:
                progress.update(prog_id, description="[cyan]외부 서비스 동기화 중...")
                from api.server import _sync_task
                _sync_task(task, session, notion, google, apple)

            progress.update(prog_id, completed=1)

            # 세션 닫히기 전에 필요한 데이터 스냅샷
            task_snapshot = {
                "id": task.id,
                "title": task.title,
                ...
                "subtasks": [
                    {"order": s.order, "title": s.title, "estimated_hours": s.estimated_hours}
                    for s in task.subtasks
                ],
                "schedule_blocks": [
                    {"start_time": b.start_time, "end_time": b.end_time, "planned_hours": b.planned_hours}
                    for b in task.schedule_blocks[:3]
                ],
            }

    console.print()
    _print_task_panel_from_dict(task_snapshot)
...
def show_schedule(days: int = typer.Option(7, "-d", "--days", help="조회할 일수")):
    ...
    with get_session() as session:
        ...
        for b in blocks:
            ...
            table.add_row(
                time_str,
                (b.task.title[:35] if b.task else "?") + (" ↩️" * b.reschedule_count if b.reschedule_count else ""),
                str(b.planned_hours),
...
def mcp_sync(
    ...
    skip_notion: bool = typer.Option(False, "--skip-notion", help="Notion 페이로드 건너뜀"),
    skip_google: bool = typer.Option(False, "--skip-google", help="Google Calendar 페이로드 건너뜀"),
):
    """
    ...
    with get_session() as session:
        ...
        if not skip_notion and not already_notion:
        ...
        if not skip_google and task.schedule_blocks:
            ...
            if gcal_items:
                ids_example = ", ".join(
                    '{{"block_id": {}, "event_id": "<반환값>"}}'.format(i["block_id"])
                    for i in gcal_items
                )
                console.print(
                    f"\n[dim]각 블록의 event_id를 아래로 저장:[/dim]\n"
                    f"  PATCH /tasks/{task_id}/external-ids\n"
                    f'  Body: {{"google_event_ids": [{ids_example}]}}'
                )
...
def _print_task_panel(task) -> None:
    """ORM 객체를 직접 받는 버전 (세션 내부에서만 사용)."""
    _print_task_panel_from_dict({
        "id": task.id,
        "title": task.title,
        "priority": task.priority,
        "priority_score": task.priority_score,
...
def _print_task_panel_from_dict(t: dict) -> None:
    lines = [
        f"[bold]ID:[/bold] {t['id']}",
        f"[bold]제목:[/bold] {t['title']}",
        f"[bold]우선순위:[/bold] {_priority_label(t['priority'])}  (점수: {t['priority_score']})",
        f"[bold]마감일:[/bold] {t['deadline'].astimezone(tz).strftime('%Y/%m/%d %H:%M') if t['deadline'] else '없음'}",
        f"[bold]예상 시간:[/bold] {t['estimated_hours']}h",
    ]

    if t.get("subtasks"):
        lines.append(f"\n[bold]세부 작업 ({len(t['subtasks'])}개):[/bold]")
        for st in t["subtasks"]:
            lines.append(f"  {st['order']}. {st['title']} ({st['estimated_hours']}h)")

    if t.get("schedule_blocks"):
        lines.append(f"\n[bold]일정 블록 ({len(t['schedule_blocks'])}개):[/bold]")
        for b in t["schedule_blocks"]:
            lines.append(
